...
\begin{definition}[Differential of a two-variate function]
    Given $S \subseteq \real^2$ and a differentiable function $f\colon S \to \real$, then the \textit{differential} on $f$ at any point $(x, y) \in S$ is defined as
    \[
...
\begin{definition}[Exact Differential]
    Given $S \subseteq \real^2$, then a differential form $P(x, y)dx + Q(x, y)dy$ defined on $S$ is an \textit{exact differential} iff there exists some $f\colon S \to \real$ such that $df(x, y) = P(x, y)dx + Q(x, y)dy$.
\end{definition}
